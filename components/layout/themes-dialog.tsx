    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-6 py-2">
          ...
          <div>
            <p className="mb-3 text-sm font-semibold text-foreground">Color theme</p>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive ? "border-primary bg-primary/10 text-foreground ring-1 ring-primary/30" : "border-border bg-card text-muted-foreground hover:bg-accent hover:text-foreground"
                    )}
                  >
                    <span className={cn("h-5 w-5 shrink-0 rounded-full ring-2 ring-offset-2 ring-offset-background", COLOR_SWATCH[id], isActive ? "ring-primary" : "ring-transparent")} />
                    {COLOR_LABELS[id]}
                    {isActive && <Check className="ml-auto h-4 w-4 shrink-0 text-primary" strokeWidth={2.5} />}
                  </button>
                )
              })}
            </div>
